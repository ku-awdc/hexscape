...
curve(dweibull(x, rho, scale),
    from = 0, to = MaxT, ylab = "", xlab = "distance"
)

# And this is the cumulative proportion by distances travelled:
curve(pweibull(x, rho, scale),
    from = 0, to = MaxT, ylab = "", xlab = "distance"
)
...
# So the total hazard of settling between x km and y km conditional
#  on having made it as far as x km is:
(p_y - p_x) / (1 - p_x)

# And the instantaneous hazard of settling at x km conditional
# on having made it as far as x km is:
